...
function DirectionsMapComponent({ origin, destination }: DirectionsMapProps) {
    ...
    isLoaded ? (
      <div
        ...
        <div className="relative">
          {/* Small custom Map/Satellite control overlay */}
          <div
          ...
          {/* Street View (Pegman) compact button */}
            ...
            {directionsResponse && travelMode !== 'TRANSIT' && (() => {
              ...
              return (
                <div className="flex flex-col justify-end items-end gap-1 text-sm text-gray-700 ml-auto">
                  {/* <span className="flex items-center gap-1 text-orange-700"><FaClock />{leg.duration?.text}</span> */}
                  <p className='text-lg text-orange-700'>{leg.distance?.text}</p>
                </div>
              );
            })()}
          </div>
